...
/** Courier options for the FORWARD shipment of a paid order — used by the admin dispatch UI before AWB generation. */
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const deliveryPincode = (order.deliveryAddress as DeliveryAddress | null)?.pincode;
  const pickupPincode = process.env.WAREHOUSE_PINCODE;
  if (!pickupPincode || !deliveryPincode) {
    return NextResponse.json({ error: "Warehouse or delivery pincode is not available." }, { status: 400 });
  }
