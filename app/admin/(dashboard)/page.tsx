...
export default async function AdminDashboardPage() {
  ...
  return <div className="flex flex-col gap-6">
    <div className="flex flex-wrap items-center justify-between gap-4">
      ...
      <Link href="/admin/products/new" className="inline-flex items-center gap-2 rounded-xl bg-[#025632] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#013720]"><Plus size={17} aria-hidden="true" />Add product</Link>
    </div>
    ...
    <section className="relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><caption className="sr-only">Eight most recent orders</caption><thead className="border-y border-zinc-100 bg-zinc-50/80 text-[10px] uppercase tracking-wider text-zinc-500"><tr>{["Order", "Customer", "Items", "Amount", "Status", ""].map((label, i) => <th key={i} scope="col" className="px-5 py-3 font-semibold">{label || <span className="sr-only">Details</span>}</th>)}</tr></thead><tbody>
        {recentOrders.map(order => <tr key={order.id} className="border-b border-zinc-100 transition-colors last:border-0 hover:bg-zinc-50/70"><td className="px-5 py-4"><Link href={`/admin/orders/${order.id}`} className="font-mono text-xs font-medium text-zinc-700 hover:text-[#059669]">{formatOrderNumber(order.createdAt)}</Link><p className="mt-1 text-[11px] text-zinc-400">{new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric", timeZone: "Asia/Kolkata" }).format(order.createdAt)}</p></td><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#ecfdf5] text-[10px] font-semibold text-[#047857]" aria-hidden="true">{order.customerName.split(" ").filter(Boolean).slice(0, 2).map(n => n[0]).join("")}</span><span className="font-medium">{order.customerName}</span></div></td><td className="px-5 py-4 text-xs text-zinc-500">{order.items.length} {order.items.length === 1 ? "item" : "items"}</td><td className="whitespace-nowrap px-5 py-4 font-semibold tabular-nums">{formatInr(order.amount)}</td><td className="px-5 py-4"><span className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 py-1 text-[10px] font-medium ${STATUS_STYLES[order.status] ?? "bg-zinc-100 text-zinc-700"}`}><span className="size-1.5 rounded-full bg-current" />{order.status.toLowerCase().replaceAll("_", " ").replace(/^./, c => c.toUpperCase())}</span></td><td className="px-5 py-4"><Link href={`/admin/orders/${order.id}`} aria-label={`View order ${formatOrderNumber(order.createdAt)}`} className="inline-flex size-8 items-center justify-center rounded-lg border border-zinc-200 text-zinc-500 hover:border-[#a7f3d0] hover:bg-[#ecfdf5] hover:text-[#059669]"><ArrowUpRight size={16} aria-hidden="true" /></Link></td></tr>)}
...
function StatCard({ label, value, detail, icon: Icon, href, warn, featured }: { label: string; value: string; detail: string; icon: LucideIcon; href: string; warn?: boolean; featured?: boolean }) {
  return <Link href={href} className={`group min-w-0 rounded-xl border p-4 shadow-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#10b981] ${featured ? "border-[#24272d] bg-[#24272d] text-white hover:bg-[#30343b]" : "border-zinc-200/80 bg-white hover:border-[#a7f3d0]"}`}><div className="flex items-center justify-between gap-2"><span className={`text-xs font-medium ${featured ? "text-zinc-300" : "text-zinc-500"}`}>{label}</span><Icon size={17} aria-hidden="true" className={`shrink-0 ${featured ? "text-[#34d399]" : warn ? "text-amber-500" : "text-zinc-400"}`} /></div><p className={`mt-3 break-words text-2xl font-semibold leading-tight tracking-tight tabular-nums ${warn ? "text-amber-600" : ""}`}>{value}</p><p className={`mt-1 text-xs leading-relaxed ${featured ? "text-zinc-400" : "text-zinc-500"}`}>{detail}</p></Link>;
}
