...
export default async function AdminDashboardPage() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="flex flex-wrap items-center gap-x-6 gap-y-3 px-1 text-[11px] text-zinc-500"><span className="font-medium text-zinc-600">Connected services</span><span className="inline-flex items-center gap-2"><CreditCard size={14} aria-hidden="true" />Payments<span className={`size-1.5 rounded-full ${razorpayReady ? "bg-emerald-500" : "bg-amber-500"}`} />{razorpayReady ? "Configured" : "Setup required"}</span><span className="inline-flex items-center gap-2"><Truck size={14} aria-hidden="true" />Delivery<span className={`size-1.5 rounded-full ${porterReady ? "bg-emerald-500" : "bg-amber-500"}`} />{porterReady ? "Configured" : "Setup required"}</span></div>
  </div>;
}
